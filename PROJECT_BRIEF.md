...
Documentation edition 1.5 | Research cutoff: 11 September 2026 | Implementation status: v0.5 country leagues and repeatable seasons implemented on Windows. Latest owner scope: complete v0.5, then stop before v0.6. Retain the accepted layout and record unverified external checks. Linux remains unverified. See ROADMAP evidence.
...
v0.2 now includes paused substitutions, three formation presets, mentality/tempo/pressing controls and manual bench selection, three saved tactical setups, minute-60 opponent mentality and migration from schema-1/2/3 checkpoints. The short result report and shared keyboard/standard-gamepad navigation foundation are implemented. Windows software acceptance is verified; Linux, low-end performance and physical controller checks keep the cross-platform gate open. v0.3 now applies condition loss, condition/morale strength and training recovery. Injuries, league discipline/eligibility, mandatory incident decisions and academy shortage cover are implemented. Real added time and calibrated match probabilities are implemented, with explicit half phases and schema-7 migration. v0.4 now has generated wages, budget calculations and a balanced operating ledger in Club finances. Player contracts now support cost review and atomic renewal. Scouting, shortlists, uncertain comparison and dated calendar progression now work. Club bids, counters, player terms, free agents and reviewed registration now work, including expiry and queued window registration. Weekly AI recruitment and actionable Club news now work. Loans now preserve parent ownership, share wages and return automatically on June 30. The v0.4 Windows acceptance journey and packaged persistence smoke pass; next is the complete competition world. The first v0.5 slice now closes seasons on June 30, pays league prizes, releases expired contracts and archives results before starting July 1. Subsequent exhibition seasons have six weeks of preseason. The country world now has 176 clubs in ten active divisions, two-up/two-down movement and tier-specific finances. New careers default to that world; the eight-club exhibition and existing saves remain supported. National and continental cups are next. The latest owner direction supersedes the earlier cross-milestone authorization: complete v0.5, then stop before starting v0.6. Continue its remaining slices autonomously. Keep the accepted match presentation.
