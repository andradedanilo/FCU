...
One more match, one more signing, one more season. Favor visible consequences, readable numbers and a living career over administrative detail. Target a first match within five minutes of New Career; normal match presentation now targets a few minutes, with Play/Pause and 1x/2x/3x speed only; experienced play with delegated routines should complete a season in two to four hours. These are playtest targets, not measurements already achieved.
...
Between chances, show the match board, score, clock, commentary and statistics. Highlight selection prioritizes goals and saves and illustrates every third off-target attempt; all attempts remain in commentary/statistics. One clip is selected per delivered minute, with goal priority if events coincide. A selected event triggers a short 320x180 Canvas 2D scene: approach, shot and distinct outcome reaction. The initial audition lasts 4.8 seconds at 1x, scales with 1x/2x/3x playback, and returns to the board. Goal scenes celebrate; saves show the goalkeeper; misses show disappointment. The generic miss label is Off Target, since the engine does not measure how narrowly a shot misses. Do not simulate continuous physical movement or invent fouls/cards for artwork. Text fallback and reduced-motion support remain available.
...
Keep fullscreen launch, adaptive layout, F11 window toggle, Play/Pause, 1x/2x/3x and optional continuous half-time. No instant finish or manual minute skip. Substitutions, tactics and card/referee scenes await their mechanics gates. The owner supplied ISS Deluxe references for slim athletic silhouettes, jointed limbs, readable poses and late-16-bit shading. Original sprites now use a six-pose connected run cycle, a planted-leg kick and a dedicated keeper dive; do not return to square torsos or detached moving boots. These references guide anatomy and timing, not copied sprite sheets. Review the result before expanding artwork. Update these existing reference documents as decisions evolve; distinguish implemented behavior from future concepts.
