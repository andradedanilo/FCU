...
Work in reviewable visual batches. The owner approved batch 1: the title screen, club selection, clubhouse tiles, squad board, broadcast match layout, speed buttons, horizontal full-pitch camera, optional original synthesized music/SFX and a brief dismissible goal celebration. This is the accepted visual baseline. Keep keyboard focus and reduced-motion support. Do not expand gameplay merely to demonstrate an animation. Maintain these existing specifications as decisions evolve; distinguish accepted direction, implemented behavior and future concepts.

The owner approved the first retro batch and authorized the stadium batch. Batch 2 implements tiered stands, original fictional advertising boards, instanced club-colored crowds and waving flags, plus short linked pass/run/shot/save sequences and supporting player movements. A paused Replay play action runs a six-second presentation-only sequence of the latest event; automatic playback remains compressed at the selected speed. Crowd/flag motion stops on pause or while hidden. Fictional boards are scenery, not paid advertising or network content. Review pacing and framing before richer artwork. A subsequent event-storytelling batch can introduce original 2D close-ups, such as a referee showing a card and a player's reaction, driven only by actual committed events when their rules arrive. Goals already exist; fouls/cards are a later mechanics gate, penalties follow cup support, and offside/cancelled-goal/VAR concepts remain future vision rather than implemented rules. Audio direction is original short chiptune phrases, football impacts, whistle/crowd textures and simple celebration fanfares, with independent controls and no copied recordings. Human listening and visual feedback decide the next iteration.
...
The next owner review requested a grander pitch-side view, recognizable small human bodies, retro crowd arm waves and fluid play continuity. Batch 3 starts fullscreen (F11 toggles window mode), fills the available display, and frames the full pitch while outer stadium decks may extend beyond the camera. Players carry their previous positions into each play instead of snapping to formation. Highlight duration is six seconds at 1x; quiet minutes use 1.2 seconds, both divided by the selected 1x/2x/3x speed. Substitutions and tactics are still deferred. Half-time continuation is a session option and never bypasses future mandatory decisions.
