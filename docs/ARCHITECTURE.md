...
No Three.js or WebGL runtime is retained. Commentary is always the main view; animated pixel cutaways temporarily cover it. Reduced motion disables the cutaways. Canvas initialization failure leaves commentary visible with a notice. Stop animation on hidden/unmount and freeze actual clips on pause. Explicit art previews are presentation-only, run for 4.8 seconds and never write to the worker or save store. End or dismiss a clip by returning to central commentary. Do not autoplay old events on load.
