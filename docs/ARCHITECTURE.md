...
v0.2 first slice uses schema 2 and engine/app 0.2.0. Schema-1 saves are validated with the legacy schema before explicit migration; active benches are derived from their original lineups, with no substitutions. Checksum validation precedes migration. New saves require all bench/history fields; they are not defaulted for malformed schema-2 input. Save history links record the original and new versions; no old checkpoint is rewritten.
