...
function App(){
  ...
  const clubName=(id:string|null)=>roster?.teams.find(team=>team.id===id)?.displayName??view?.previousTeams.find(team=>team.id===id)?.displayName??t.noClub;
  const changes=view?.changes.filter(change=>change.from===team?.id||change.to===team?.id)??[];
  return <main className={s.app}><header><h1>{t.title}</h1><p>{t.subtitle}</p></header><p className={s.notice}>{t.notice}</p>
    ...
    <section className={s.board}><aside><h2>{t.clubs}</h2>{roster.teams.map(team=><button key={team.id} aria-pressed={team.id===(selected||roster.teams[0]?.id)} onClick={()=>setSelected(team.id)}>{team.displayName}<small>{view.reviewedTeamIds.includes(team.id)?t.reviewed:t.pending}</small></button>)}</aside>
      ...
      <section><h2>{t.changes}</h2><p>{view.previousSnapshotId?`${t.baseline}: ${view.previousSnapshotId}`:t.firstExport}</p>{changes.length?<ul>{changes.map(change=><li key={change.playerId}><strong>{t[change.kind]}: {change.name}</strong><small>{clubName(change.from)} -&gt; {clubName(change.to)}</small></li>)}</ul>:<p>{t.noChanges}</p>}<h2>{t.issues}</h2>{view.provider==='fictional'&&<button disabled={busy||view.reviewedTeamIds.length===roster.teams.length} onClick={()=>void act({type:'reviewFixture',hash:view.hash!})}>{t.reviewFixture}</button>}{view.issues.length?<ul>{view.issues.map((issue,i)=><li key={i}>{issue.code}: {issue.message}</li>)}</ul>:<p>{t.noIssues}</p>}</section>
    </section></>}
  </main>;
}
