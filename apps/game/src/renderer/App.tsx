...
export function App() {
  ...
  return <div className={styles.app}>
    ...
    <main className={styles.main}>
      <header className={styles.topbar}><span>{t.season}{state&&` / ${state.date}`}</span><div>{state&&<small>{dirty?t.unsaved:t.savedStatus}</small>}<button disabled={busy} onClick={()=>{stop();setSetup(true);}}>{t.newCareer}</button><button disabled={busy} onClick={()=>void showSaves()}>{t.load}</button>{state&&<button disabled={busy} onClick={()=>{stop();setBusy(true);occupied.current=true;void save().finally(()=>{occupied.current=false;setBusy(false);});}}>{t.save}</button>}</div></header>
      ...
      state&&<>
        ...
        {screen==='match'&&<>{match?<><div className={styles.matchHeading}><p className={styles.eyebrow}>{t.match} / {t.round} {match.tick===90?state.round:state.round+1}</p><span>{match.tick===90?t.final:playing?t.live:t.paused}</span></div><div className={styles.scoreboard}><h2>{name(match.home)}</h2><div><strong data-testid="score">{match.homeGoals} - {match.awayGoals}</strong><span data-testid="minute">{match.tick===90?t.fullTime:match.tick===45?t.halfTime:`${match.tick}'`}</span></div><h2>{name(match.away)}</h2></div><div className={styles.actions}>{match.tick<90?<><button className={styles.primary} disabled={busy} onClick={play}>{playing?t.pause:match.tick===45?t.continue:t.play}</button><button disabled={busy||playing} onClick={()=>void command({type:'AdvanceMatch',minutes:1})}>{t.minute}</button><button disabled={busy||playing} onClick={()=>void command({type:'AdvanceMatch',minutes:90})}>{t.finish}</button><label>{t.speed}<select value={speed} onChange={e=>{setSpeed(Number(e.target.value));speedRef.current=Number(e.target.value);}}><option value={1}>1x</option><option value={3}>3x</option><option value={8}>8x</option></select></label></>:<button className={styles.primary} onClick={()=>navigate('home')}>{t.continue} →</button>}</div><MatchVisual state={state} playing={playing} speed={speed}/><div className={styles.matchStats}>{([['shots',t.shots],['onTarget',t.onTarget],['possession',t.possession],['quality',t.quality]] as const).map(([key,label])=>{const format=(n:number)=>key==='quality'?(n/10000).toFixed(2):key==='possession'?`${match.tick?Math.round(n/match.tick/100):50}%`:n;return <div key={key}><strong>{format(match.homeStats[key])}</strong><span>{label}</span><strong>{format(match.awayStats[key])}</strong></div>;})}</div><section className={styles.commentary}><h3>{t.events}</h3>{match.events.length===0?<p>{t.noEvents}</p>:<ol>{[...match.events].reverse().map(e=><li key={e.order} className={e.type==='goal'?styles.goal:undefined}>{describeEvent(state,e)}</li>)}</ol>}</section></>:<><h1>{t.matchIntro}</h1><p>{t.matchEmpty}</p></>}</>}
      </>}
