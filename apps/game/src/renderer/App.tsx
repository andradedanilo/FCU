...
type Action = {type:'StartMatch'} | {type:'SelectLineup';lineup:PlayerId[]} | {type:'AdvanceMatch';minutes:number} | {type:'Substitute';out:PlayerId;in:PlayerId} | {type:'SetTactics';tactics:Tactics};
export function App() {
  ...
  async function command(action:Action) {
    ...
    if(result.ok) {
      accept(result.value,action.type==='SetTactics');setDirty(true);
  ...
  return <div className={s.app} onClickCapture={event=>{if(event.target instanceof Element&&event.target.closest('button'))audio.current?.click();}}>
    <div className={s.cabinet}>
      ...
      <main className={s.gameScreen} key={screen}>
        ...
        {state&&screen==='squad'&&<SquadScreen tactics={()=>{stop();setTacticsOpen(true);}} state={state} lineup={lineup} change={setLineup} suggest={()=>setLineup(autoPick(state.players,state.clubId,state.tactics.formation))} confirm={()=>void command({type:'SelectLineup',lineup})} busy={busy}/>}
    ...
    {state&&tacticsOpen&&<TacticsMenu draftLineup={screen==='squad'?lineup:state.lineup} state={state} busy={busy} close={()=>setTacticsOpen(false)} confirm={tactics=>command({type:'SetTactics',tactics})}/>}
