...
export function ScoutingMenu({state,busy,command,close}:{state:Career;busy:boolean;command:(action:ScoutAction)=>Promise<boolean>;close:()=>void}){
 ...
 const candidates=state.players.filter(p=>p.clubId!==state.clubId&&!p.academy&&(role==='all'||p.role===role)&&(!shortlisted||state.scouting.shortlist.includes(p.id)));
