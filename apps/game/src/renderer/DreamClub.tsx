...
export function DreamClub({packs,exit,cue,ambience}:{packs:InstalledRoster[];exit:()=>void;cue:(kind:MatchCue)=>void;ambience:(value:boolean)=>void}){
 ...
 const g=state?.game;
 return <section className={d.root}><header className={s.sectionTitle}><h1>{brand.dreamClub}</h1><div><button data-input-back disabled={busy} onClick={()=>{stop();exit();}}>{t.back}</button><button disabled={busy} onClick={()=>void list()}>{t.load}</button>{state&&<button disabled={busy} onClick={()=>{stop();if(!occupied.current){occupied.current=true;setBusy(true);void persist(state).finally(()=>{occupied.current=false;setBusy(false);});}}}>{t.save}</button>}</div></header><p role="status">{notice}</p>
