...
export function BenchMenu({state,busy,confirm,close}:{state:Career;busy:boolean;confirm:(ids:PlayerId[])=>Promise<boolean>;close:()=>void}){
 const [draft,setDraft]=useState(state.bench.filter(id=>eligible(state.players.find(p=>p.id===id)!,state.date)));const dialog=useRef<HTMLDialogElement>(null);
 useModal(dialog);
 const available=state.players.filter(p=>p.clubId===state.clubId&&!state.lineup.includes(p.id)&&eligible(p,state.date));
