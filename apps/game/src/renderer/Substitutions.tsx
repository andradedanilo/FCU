...
export function Substitutions({state,busy,confirm,close}:{state:Career;busy:boolean;confirm:(out:PlayerId,incoming:PlayerId)=>Promise<boolean>;close:()=>void}) {
 ...
 const available=bench.filter(id=>!lineup.includes(id)&&!changes.some(c=>c.out===id));
 ...
 return <dialog ref={dialog} className={s.dialog} onCancel={event=>{if(busy)event.preventDefault();else close();}} aria-label={t.substitutions}>
  ...
  <div className={s.lists}>{([{title:t.onPitch,ids:lineup,selected:out,choose:(id:PlayerId)=>{setOut(id);setIncoming(null);},bench:false},{title:t.matchBench,ids:available,selected:incoming,choose:setIncoming,bench:true}]).map(group=><section key={group.title}><h3>{group.title}</h3><div role="group" aria-label={group.title}>{group.ids.map(id=>{const p=state.players.find(p=>p.id===id)!;const eligible=!group.bench||(outgoing&&(outgoing.role==='GK')===(p.role==='GK'));return <button key={id} aria-pressed={id===group.selected} disabled={busy||!allowed||!eligible} onClick={()=>group.choose(id)}><small>{p.role}</small><span>{p.name}</span><b title={t.ability}>{overall(p)}</b><small title={t.condition}>{Math.round(match.condition[id]!/1000)}%</small></button>;})}</div></section>)}</div>
