...
export type MatchCue=HighlightKind|'anticipation'|'kick'|'whistle'|'tackle'|'fulltime';
...
export function useMatchBroadcast(state:Career,playing:boolean,cue:(kind:MatchCue)=>void){
 ...
 useEffect(()=>{
  ...
  if(!playback.current.started){if(sequence.frame!=='tackle')sound.current(sequence.frame==='prepare'||sequence.frame==='penalty'?'anticipation':sequence.highlight.kind);playback.current.started=true;}
