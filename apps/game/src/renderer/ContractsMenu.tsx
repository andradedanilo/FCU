...
export function ContractsMenu({state,busy,confirm,close,initialPlayer}:{initialPlayer:PlayerId|null;state:Career;busy:boolean;confirm:(action:Renewal|OutgoingAction)=>Promise<boolean>;close:()=>void}){
 ...
 const contract=state.contracts[player.id]!,wage=parseEuro(draft.wage);
 const action:Renewal={type:player.academy?'PromoteAcademy':'RenewContract',playerId:player.id,contractRevision:contract.revision,years:draft.years,wage:wage??0,bonus:4*(wage??0),role:draft.role};
 ...
 return <dialog ref={dialog} className={s.dialog} aria-label={t.contracts} onCancel={close}><header><h2>{t.contracts}</h2><button disabled={busy} onClick={close}>{t.close}</button></header>
  <div className={s.body}><div className={s.players} role="group" aria-label={t.contractPlayers}>{players.map(p=><button key={p.id} disabled={busy||review} aria-pressed={p.id===draft.id} onClick={()=>setDraft(draftFor(p))}><strong>{p.name}</strong><small>{p.academy?t.academyPlayer:p.role} / {state.contracts[p.id]!.ends}</small><span>{money(state.economy.wages[p.id]!)}</span></button>)}</div>
