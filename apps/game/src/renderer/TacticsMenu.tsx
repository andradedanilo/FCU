...
export function TacticsMenu({state,store,draftLineup,busy,confirm,close}:{state:Career;store:(slot:number,tactics:Tactics|null)=>Promise<boolean>;draftLineup:PlayerId[];busy:boolean;confirm:(tactics:Tactics)=>Promise<boolean>;close:()=>void}) {
 ...
 return <dialog ref={dialog} className={s.dialog} aria-label={t.tactics} onCancel={e=>{if(busy)e.preventDefault();else close();}}>
  ...
  <div className={s.body}><section data-input-section><h3>{t.formation}</h3><div className={s.options} role="group" aria-label={t.formation}>{formationSchema.options.map(formation=><button key={formation} disabled={busy} aria-pressed={draft.formation===formation} onClick={()=>setDraft({...draft,formation})}>{formation}</button>)}</div>
  ...
  <section data-input-section>{groups.map(group=><div className={s.control} key={group.key}><h3>{t[group.key]}</h3><div className={s.options} role="group" aria-label={t[group.key]}>{group.options.map(value=><button key={value} disabled={busy} aria-pressed={draft[group.key]===value} onClick={()=>setDraft({...draft,[group.key]:value})}>{t.choices[value]}</button>)}</div><p>{group.hint}</p></div>)}</section></div>
