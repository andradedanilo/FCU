...
export function MatchScreen({state,acknowledge,report,tactics,busy,playing,play,pause,substitute,continuousHalf,changeContinuous,done,goal}:{state:Career;acknowledge:()=>void;report:()=>void;tactics:()=>void;busy:boolean;playing:boolean;play:()=>void;pause:()=>void;substitute:(out:PlayerId,incoming:PlayerId)=>Promise<boolean>;continuousHalf:boolean;changeContinuous:(value:boolean)=>void;done:()=>void;goal:(kind:'goal'|'save'|'shot')=>void}) {
 ...
 return <section data-decision={needsDecision(match)} className={s.matchScreen}>
  ...
  <div data-input-section className={s.matchControls}>{match.phase!=='finished'?<><div className={s.playControls}><button data-input-pause className={s.primary} disabled={busy||needsDecision(match)} onClick={play}>{playing?t.pause:match.phase==='interval'?t.continue:t.play}</button><button disabled={busy} onClick={tactics}>{t.tactics}</button><button disabled={busy||match.tick===0} onClick={()=>{pause();setChanges(true);}}>{t.substitutions}</button><label><input type="checkbox" checked={continuousHalf} onChange={event=>changeContinuous(event.target.checked)}/>{t.continuousHalf}</label></div><span>{needsDecision(match)?<><span>{match.pendingInjuries.length?t.injuryDecision:t.dismissDecision}</span><button disabled={busy} onClick={acknowledge}>{match.pendingInjuries.length?t.acknowledge:t.acknowledgeCard}</button></>:match.phase==='interval'?t.halfTime:t.fixedSpeed}</span></>:<><strong>{summary&&`${match.forfeit?t.forfeited:t[summary.outcome]} / +${summary.earned} ${t.points} / #${summary.position}`}</strong><button onClick={report}>{t.matchReport}</button><button className={s.primary} onClick={done}>{t.continue} <span aria-hidden="true">▶</span></button></>}</div>
