...
const envelope=z.strictObject({schema:z.number().int().min(1).max(4),appVersion:z.string().regex(/^\d+\.\d+\.\d+$/).max(20).optional(),gameMode:z.literal('dreamClub'),commit:z.string().uuid(),parent:z.string().uuid().nullable(),date:z.iso.datetime(),checksum:z.string().regex(/^[a-f0-9]{64}$/),payload:z.unknown()});
export function decodeDream(bytes:Uint8Array){
 ...
 if(typeof raw==='object'&&raw!==null&&'schema' in raw&&typeof raw.schema==='number'&&raw.schema>4)throw Error('FUTURE_SAVE');
