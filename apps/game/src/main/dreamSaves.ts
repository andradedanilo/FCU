...
const envelope=z.strictObject({schema:z.literal(1),gameMode:z.literal('dreamClub'),commit:z.string().uuid(),parent:z.string().uuid().nullable(),date:z.iso.datetime(),checksum:z.string().regex(/^[a-f0-9]{64}$/),payload:z.unknown()});
export function decodeDream(bytes:Uint8Array){
 ...
 if(typeof raw==='object'&&raw!==null&&'schema' in raw&&typeof raw.schema==='number'&&raw.schema>1)throw Error('FUTURE_SAVE');
 const e=envelope.parse(raw);if(checksum(e.payload)!==e.checksum)throw Error('INVALID_SAVE');return {e,state:validateDream(e.payload)};
}
export function createDreamStore(root:string){
 ...
 function save(input:unknown):Promise<string>{
  const task=pending.then(async()=>{const state=validateDream(input),id=state.game.careerId,commit=randomUUID();await mkdir(folder(id),{recursive:true});
   const e={schema:1,gameMode:'dreamClub',commit,parent:parents.get(id)??null,date:new Date().toISOString(),checksum:checksum(state),payload:state};
