...
import { validateCareer, migrateLegacyCareer, migratePreviousCareer, migrateTacticalCareer, migratePlanningCareer, migrateConditionCareer, migrateAvailabilityCareer, migrateTimedCareer, migrateFinancialCareer } from '../../../../packages/simulation/src/engine.ts';

const MAX_BYTES=4*1024*1024;
export const checksum=(value:unknown)=>createHash('sha256').update(canonical(value)).digest('hex');
const envelopeSchema=z.object({schema:z.union([z.literal(1),z.literal(2),z.literal(3),z.literal(4),z.literal(5),z.literal(6),z.literal(7),z.literal(8),z.literal(9)]),appVersion:z.enum(['0.1.0','0.2.0','0.2.1','0.2.2','0.3.0','0.3.1','0.3.2','0.4.0','0.4.1']),engineVersion:z.enum(['0.1.0','0.2.0','0.2.1','0.2.2','0.3.0','0.3.1','0.3.2','0.4.0','0.4.1']),rulesetVersion:z.enum(['exhibition-1','exhibition-2','exhibition-3','exhibition-4','exhibition-5','exhibition-6','exhibition-7','exhibition-8']),careerId:z.string().uuid(),saveCommitId:z.string().uuid(),parentCommitId:z.string().uuid().nullable(),stateRevision:z.number().int().nonnegative(),savedAtUTC:z.string().datetime(),snapshotId:z.literal('fictional-2026-v1'),kind:z.enum(['manual','auto']),checksum:z.string().regex(/^[a-f0-9]{64}$/),payload:z.unknown()});
export function decode(bytes:Uint8Array) {
  ...
  if(typeof raw==='object'&&raw!==null&&'schema' in raw&&typeof raw.schema==='number'&&raw.schema>9)throw new Error('FUTURE_SAVE');
  const e=envelopeSchema.parse(raw);
  if(checksum(e.payload)!==e.checksum)throw new Error('INVALID_SAVE');
  if((e.schema===9&&(e.engineVersion!=='0.4.1'||e.appVersion!=='0.4.1'||e.rulesetVersion!=='exhibition-8'))||(e.schema===8&&(e.engineVersion!=='0.4.0'||e.appVersion!=='0.4.0'||e.rulesetVersion!=='exhibition-7'))||(e.schema===7&&(e.engineVersion!=='0.3.2'||e.appVersion!=='0.3.2'||e.rulesetVersion!=='exhibition-6'))||(e.schema===6&&(e.engineVersion!=='0.3.1'||e.appVersion!=='0.3.1'||e.rulesetVersion!=='exhibition-5'))||(e.schema===5&&(e.engineVersion!=='0.3.0'||e.appVersion!=='0.3.0'||e.rulesetVersion!=='exhibition-4'))||(e.schema===4&&(e.engineVersion!=='0.2.2'||e.appVersion!=='0.2.2'||e.rulesetVersion!=='exhibition-3'))||(e.schema===3&&(e.engineVersion!=='0.2.1'||e.appVersion!=='0.2.1'||e.rulesetVersion!=='exhibition-2'))||(e.schema<3&&e.rulesetVersion!=='exhibition-1')||(e.schema===1&&(e.engineVersion!=='0.1.0'||e.appVersion!=='0.1.0'))||(e.schema===2&&(e.engineVersion!=='0.2.0'||e.appVersion!=='0.2.0')))throw new Error('INVALID_SAVE');
  const state=e.schema===1?migrateLegacyCareer(e.payload):e.schema===2?migratePreviousCareer(e.payload):e.schema===3?migrateTacticalCareer(e.payload):e.schema===4?migratePlanningCareer(e.payload):e.schema===5?migrateConditionCareer(e.payload):e.schema===6?migrateAvailabilityCareer(e.payload):e.schema===7?migrateTimedCareer(e.payload):e.schema===8?migrateFinancialCareer(e.payload):validateCareer(e.payload);
