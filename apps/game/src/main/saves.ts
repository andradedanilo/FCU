...
import { validateCareer, migrateLegacyCareer, migratePreviousCareer } from '../../../../packages/simulation/src/engine.ts';

const MAX_BYTES=4*1024*1024;
export const checksum=(value:unknown)=>createHash('sha256').update(canonical(value)).digest('hex');
const envelopeSchema=z.object({schema:z.union([z.literal(1),z.literal(2),z.literal(3)]),appVersion:z.enum(['0.1.0','0.2.0','0.2.1']),engineVersion:z.enum(['0.1.0','0.2.0','0.2.1']),rulesetVersion:z.enum(['exhibition-1','exhibition-2']),careerId:z.string().uuid(),saveCommitId:z.string().uuid(),parentCommitId:z.string().uuid().nullable(),stateRevision:z.number().int().nonnegative(),savedAtUTC:z.string().datetime(),snapshotId:z.literal('fictional-2026-v1'),kind:z.enum(['manual','auto']),checksum:z.string().regex(/^[a-f0-9]{64}$/),payload:z.unknown()});
export function decode(bytes:Uint8Array) {
  ...
  if(typeof raw==='object'&&raw!==null&&'schema' in raw&&typeof raw.schema==='number'&&raw.schema>3)throw new Error('FUTURE_SAVE');
  const e=envelopeSchema.parse(raw);
  if(checksum(e.payload)!==e.checksum)throw new Error('INVALID_SAVE');
  if((e.schema===3&&(e.engineVersion!=='0.2.1'||e.appVersion!=='0.2.1'||e.rulesetVersion!=='exhibition-2'))||(e.schema<3&&e.rulesetVersion!=='exhibition-1')||(e.schema===1&&(e.engineVersion!=='0.1.0'||e.appVersion!=='0.1.0'))||(e.schema===2&&(e.engineVersion!=='0.2.0'||e.appVersion!=='0.2.0')))throw new Error('INVALID_SAVE');
  const state=e.schema===1?migrateLegacyCareer(e.payload):e.schema===2?migratePreviousCareer(e.payload):validateCareer(e.payload);
