...
export function protectClose(window:BrowserWindow,trusted:(event:IpcMainEvent)=>boolean,idle:()=>Promise<unknown>){
 let closing=false,prompting=false,pending:{id:string;timer:ReturnType<typeof setTimeout>}|null=null;
 ...
 async function failed(){
  ...
  try{const result=await dialog.showMessageBox(window,{type:'warning',message:t.closeSaveFailed,detail:t.closeSaveHint,buttons:[t.closeRetry,t.closeCancel,t.closeDiscard],defaultId:0,cancelId:1,noLink:true});if(window.isDestroyed())return;prompting=false;if(result.response===0)request();else if(result.response===2)window.destroy();}finally{prompting=false;}
 }
 function request(){
  if(pending||prompting||window.isDestroyed())return;const id=randomUUID();
