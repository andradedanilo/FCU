...
export type Match=z.infer<typeof incidentMatchSchema>;
export const careerSchema=conditionCareerSchema.extend({engineVersion:z.literal('0.3.1'),rulesetVersion:z.literal('exhibition-5'),lineup:z.array(playerId).min(7).max(11),players:z.array(availablePlayerSchema).min(176).max(240),bench:z.array(playerId).max(9),match:incidentMatchSchema.nullable()});
