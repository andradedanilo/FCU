...
export function matchReport(state:Career){
 ...
 return {outcome:own>other?'victory' as const:own<other?'defeat' as const:'draw' as const,earned:own>other?3:own===other?1:0,position:table.indexOf(row)+1,points:row.points,scorers:match.events.filter(e=>e.type==='goal').map(e=>({tick:matchMinute(e.tick,match.addedTime[0]??0),player:state.players.find(p=>p.id===e.playerId)!.name,club:state.clubs.find(c=>c.id===e.clubId)!.short})),own:home?match.homeStats:match.awayStats,other:home?match.awayStats:match.homeStats};
}
