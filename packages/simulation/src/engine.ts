...
export function migrateFacilitiesCareer(input:unknown):Career {const old=facilitiesCareerSchema.parse(input);return validateCareer({...old,board:initialBoard(old),engineVersion:'0.7.3',marketArchive:[],personnel:initialPersonnel(old.players,old.clubs,old.date)});}
export function migratePersonnelCareer(input:unknown):Career {const old=personnelCareerSchema.parse(input);return validateCareer({...old,engineVersion:'0.7.3',marketArchive:[],board:initialBoard(old)});}
export function migrateBoardCareer(input:unknown):Career {const old=boardCareerSchema.parse(input);return validateCareer({...old,engineVersion:'0.7.3',marketArchive:[]});}
