import {type Career,type InstalledRoster,type ClubId,type Player,clubSchema,playerId} from '../../contracts/src/index.ts';
...
export function createPackedCareer(careerId:string,seed:number,clubId:ClubId,pack:InstalledRoster):Career {
  ...
  for(const team of pack.roster.teams){
    ...
    if(squad.length<18||squad.length>30||squad.some(m=>m.isLoan||m.owningTeamId!==team.id))throw Error('ROSTER_REGISTRATION_NOT_SUPPORTED');
    for(const [i,membership] of squad.entries()){
      ...
      players.push({id,clubId:destination,name:source.displayName,role:source.primaryRole,...profile.attributes,condition:100000,morale:70,academy:false,injuryUntil:null,leagueYellows:0,leagueBan:0});
    }
  }
