...
export function callUp(state:Career,club:ClubId,kind:CompetitionClass=nextCompetition(state)):boolean {
 ...
 const ability=45+state.facilities[club]!.academy*2;
 state.players.push({id:('player-'+club.slice(5)+'-'+number) as PlayerId,clubId:club,name:'Academy '+club.slice(5)+' '+number,role,registered:true,academy:true,condition:100000,morale:70,injuryUntil:null,leagueYellows:0,leagueBan:0,goalkeeping:role==='GK'?ability:15,tackling:ability,passing:ability,shooting:ability,pace:ability,stamina:ability,discipline:60});state.economy.wages[state.players.at(-1)!.id]=0;state.contracts[state.players.at(-1)!.id]=initialContract(state.players.at(-1)!,state.date);state.personnel.players[state.players.at(-1)!.id]=newPerson(state.players.at(-1)!);return true;
}
