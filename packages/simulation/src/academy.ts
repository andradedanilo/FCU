...
export function callUp(state:Career,club:ClubId,kind:CompetitionClass=nextCompetition(state)):boolean {
 ...
 state.players.push({id:('player-'+club.slice(5)+'-'+number) as PlayerId,clubId:club,name:'Academy '+club.slice(5)+' '+number,role,registered:true,academy:true,condition:100000,morale:70,injuryUntil:null,leagueYellows:0,leagueBan:0,goalkeeping:role==='GK'?45:15,tackling:45,passing:45,shooting:45,pace:45,stamina:45,discipline:60});state.economy.wages[state.players.at(-1)!.id]=0;state.contracts[state.players.at(-1)!.id]=initialContract(state.players.at(-1)!,state.date);return true;
}
