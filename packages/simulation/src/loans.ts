...
export function occupiedPlaces(state:Career,club:ClubId){return state.players.filter(p=>p.clubId===club&&!p.academy&&p.registered).length+state.loans.filter(l=>l.parent===club&&l.status==='active').length;}
...
export function returnLoans(state:Career){
 ...
 for(const loan of state.loans){
  ...
  const player=state.players.find(p=>p.id===loan.playerId)!;player.clubId=loan.parent;player.registered=true;state.contracts[player.id]!.revision++;loan.status='returned';changed=true;
...
export function validateLoans(state:Career){
 ...
 for(const loan of state.loans){
  ...
  if(ids.has(loan.id)||!player||!contract||loan.parent===loan.borrower||![loan.parent,loan.borrower].every(id=>state.clubs.some(c=>c.id===id))||!validDate(loan.ends)||(loan.source==='market'&&(!offer||offer.status!=='completed'||offer.loanShare!==loan.share||offer.playerId!==loan.playerId||offer.buyerId!==loan.borrower||offer.sellerId!==loan.parent))||(loan.source==='roster'&&!state.rosterOrigin?.playerIds[loan.playerId]))throw Error('INVALID_SAVE');
