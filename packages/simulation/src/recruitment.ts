...
export function recruit(state:Career){
 ...
 for(const club of [...state.clubs].sort((a,b)=>a.id<b.id?-1:1)){
  ...
  for(let slot=existing;slot<2;slot++){
   ...
   const candidates=state.players.filter(p=>state.personnel.players[p.id]!.retired===null&&p.clubId!==club.id&&p.clubId!==state.clubId&&!p.academy&&p.role===role&&ageOn(state.contracts[p.id]!.birthDate,state.date)<34&&!pending.some(o=>o.playerId===p.id)).map(player=>{const ability=estimate(view,player,3);return {player,ability:ability.low+ability.high,age:ageOn(state.contracts[player.id]!.birthDate,state.date)};}).sort((a,b)=>b.ability-a.ability||a.age-b.age||(a.player.id<b.player.id?-1:1)).map(entry=>entry.player);
