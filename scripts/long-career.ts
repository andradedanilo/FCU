...
import {maintainSquad,respondToInjuries} from './career-policy.ts';
...
if(isMainThread){
 ...
 try{await Promise.race([Promise.all(workers.map(worker=>new Promise<void>((resolve,reject)=>{worker.on('error',reject);worker.on('exit',code=>code===0?resolve():reject(Error('LONG_CAREER_WORKER')));}))),new Promise<never>((_resolve,reject)=>{timeout=setTimeout(()=>reject(Error('LONG_CAREER_BUDGET')),600000);})]);
...
for(const seed of seeds){
 ...
 for(let year=0;year<10;year++){
  while(nextManagedFixture(s)){
   job();if(maintained)maintainSquad(()=>s,act,policy);while(s.date!==nextFixtureDate(s)){act({type:'AdvanceCalendar',target:'event'});job();if(maintained)maintainSquad(()=>s,act,policy);}
  ...
  const saved=canonical(s);s=validateCareer(JSON.parse(saved));assert.equal(canonical(s),saved,'Season save roundtrip changed state');if(maintained)assert.equal(ownForfeits,0,'Maintained manager forfeited: '+seed+' season '+s.season);assert(s.players.length<60000,'Population exceeded supported capacity');
