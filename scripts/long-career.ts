...
import {maintainSquad,respondToInjuries,manageMarket,tradingMetrics} from './career-policy.ts';
...
const started=performance.now();const maintained=!process.argv.includes('--depletion');
// Three fixed seeds, ten full country-world seasons each, with explicit manager policy.
if(isMainThread){
 const workers:Worker[]=[];
 const run=async()=>{for(const seed of [2026,2027,2028]){const worker=new Worker(new URL(import.meta.url),{workerData:seed});workers.push(worker);worker.on('message',(report:unknown)=>process.stdout.write(JSON.stringify(report)+'\n'));await new Promise<void>((resolve,reject)=>{worker.on('error',reject);worker.on('exit',code=>code===0?resolve():reject(Error('LONG_CAREER_WORKER')));});}};
 let timeout:ReturnType<typeof setTimeout>|undefined;
 try{await Promise.race([run(),new Promise<never>((_resolve,reject)=>{timeout=setTimeout(()=>reject(Error('LONG_CAREER_BUDGET')),600000);})]);
 process.stdout.write(JSON.stringify({completedSeeds:3,seasonsEach:10,execution:'sequential',wallSeconds:(performance.now()-started)/1000,rssMB:Math.round(process.memoryUsage().rss/1048576)})+'\n');
...
for(const seed of seeds){
 ...
 const policy={renewals:0,signings:0,upgrades:0,substitutions:0};let ownForfeits=0;
 ...
 for(let year=0;year<10;year++){
  while(nextManagedFixture(s)){
   job();if(maintained){manageMarket(()=>s,act,trading);maintainSquad(()=>s,act,policy);}while(s.date!==nextFixtureDate(s)){act({type:'AdvanceCalendar',target:'event'});job();if(maintained){manageMarket(()=>s,act,trading);maintainSquad(()=>s,act,policy);}}
   ...
   else {act({type:'SelectLineup',lineup});act({type:'StartMatch'});while(s.match!.phase!=='finished'){if(maintained)respondToInjuries(()=>s,act,policy);else if(s.match!.pendingDismissal||s.match!.pendingInjuries.length)act({type:'AcknowledgeMatch'});act({type:'AdvanceMatch',minutes:90});}}
   if(s.match){managedGames++;ownForfeits+=Number(s.match.forfeit===s.clubId);managedForfeits+=Number(s.match.forfeit!==null);managedCards+=s.match.events.filter(e=>e.type==='yellow'||e.type==='red'||e.type==='secondYellow').length;managedInjuries+=s.match.injuries.length;}
   job();
  }
