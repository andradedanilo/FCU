...
export function checkTestBudget(){
 ...
 if(!units.length||!journeys)throw Error(`Test discovery empty: ${total} cases / ${files.size} files / ${journeys} Electron journeys.`);
 console.log(`Test inventory: ${total} registered cases, ${files.size} files, ${journeys} Electron journeys (no count ceilings).`);
}
