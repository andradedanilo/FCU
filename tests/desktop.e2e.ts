...
test('offline career, lineup, text/3D equivalence, WebGL recovery and full exhibition season',async()=>{
  ...
  try {
    ...
    await expect(page.locator('canvas')).toHaveCount(1);await page.getByRole('group',{name:'Speed',exact:true}).getByRole('button',{name:'3x Quick play'}).click();await expect(page.getByRole('button',{name:'3x Quick play'})).toHaveAttribute('aria-pressed','true');await page.getByRole('button',{name:'1x Matchday'}).click();await save();const initialEntries=await entries();if(!initialEntries.ok)throw new Error();const checkpoint=initialEntries.value[0]!;
    await page.screenshot({path:'work/match-3d.png'});
    await click('Finish half');await expect(page.getByTestId('minute')).toHaveText('Half-time');await expect(page.getByLabel('Event celebration')).toBeVisible();await click('Skip celebration');await expect(page.getByLabel('Event celebration')).toHaveCount(0);await save();await app.close();
    ...
    for(let round=1;round<14;round++){
      await page.getByRole('button',{name:'Kick off'}).click();await click('Text');await click('Finish half');await expect(page.getByTestId('minute')).toHaveText('Half-time');await click('Finish half');await expect(page.getByTestId('minute')).toHaveText('Full time');await page.getByRole('button',{name:'Continue',exact:false}).click();
    }
