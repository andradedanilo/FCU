...
it('records the writer app separately from the unchanged engine and preserves previous formats',async()=>{
 ...
 const bytes=await readFile(join(root,'career',state.careerId,id+'.save')),decoded=decode(bytes);expect(decoded.envelope.schema).toBe(25);expect(decoded.envelope.appVersion).toBe(manifest.version);expect(decoded.envelope.engineVersion).toBe('0.7.5');expect(checksum(decoded.state)).toBe(checksum(state));
 ...
 const raw=decodeDream(await readFile(join(root,'dream',dream.game.careerId,commit+'.dream')));expect(raw.e.schema).toBe(5);expect(raw.e.appVersion).toBe(manifest.version);expect(checksum(raw.state)).toBe(checksum(dream));
