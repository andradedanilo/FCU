...
it('rejects unavailable selection and supplies bounded fictional academy cover without healing seniors',()=>{
 ...
 const keepers=initial();keepers.players.filter(p=>p.clubId===keepers.clubId).slice(0,14).forEach(p=>{p.role='GK';});const short=autoPick(keepers.players,keepers.clubId);expect(short).toHaveLength(9);expect(applyCommand(keepers,{...base,type:'SelectLineup',lineup:short}).ok).toBe(true);expect(applyCommand(keepers,{...base,type:'CallUp'}).ok).toBe(true);
 let crisis=initial();crisis.players=crisis.players.map(p=>({...p,injuryUntil:'2026-10-01'}));
